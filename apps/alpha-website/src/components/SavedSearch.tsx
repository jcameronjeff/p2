...
export function SavedSearches() {
  ...
  return <>
    <Combobox value={searches} onChange={(val) => setSearches(val)} multiple>
      ...
      <Combobox.Options static as='div' className='space-y-0.5 flex flex-wrap sm:block'>
      {data.map((value) => (
        <Combobox.Option
          as='div'
          ...
          value={value}
        >
          <div className='font-semibold font-alt text-blue-700'>{value.name}</div>
          <div className='search-count ml-auto leading-5 h-5'>{value.count}</div>
        </Combobox.Option>
      ))}
      </Combobox.Options>
    </Combobox>

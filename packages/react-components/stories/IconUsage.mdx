...
<Canvas>
  <Story id='components-icon--all-interstate-icons' />
</Canvas>
...
We provide five distinct icon sets, the official Interstate icons as well as Prism, Solid, Outline and Mini sets from [Heroicons]((https://heroicons.com). The three Heroicons sets each provide the same list of icons with their respective styling. The main Prism icon set uses a unique list of icons that _do not_ overlap with the other sets.

```jsx
import { BellIcon } from '@prism2/icons-react/interstate'; // Interstate icon set

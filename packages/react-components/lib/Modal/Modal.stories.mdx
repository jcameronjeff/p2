...
Modals are built on top of HeadlessUI's Dialog and is built using the Dialog.Panel, Dialog.Title and Dialog.Description components.
...
<Canvas>
  <Story name="1. Succinct" id="interaction-modal--compact-example" />
</Canvas>
...
<div className="prism-dialog-frame">
  <div className='prism-dialog-backdrop backdrop-blur-sm backdrop-opacity-95 backdrop-grayscale'>
    ...
    <div className='prism-heading-2'>
      {title}
    </div>
    <div className='prism-heading-3'>
      {description}
    </div>
    <p>My dialog body content</p>
  </div>
</div>
```
...
<Canvas>
  <Story name='1. Modal' id='elements-card--autoplay' />
</Canvas>
